  ...
  if (!profile) {
    return (
      <div className="min-h-screen bg-capp-warm-bg flex flex-col items-center justify-center px-6 text-center">
        ...
        <h2 className="font-[League_Spartan] font-bold text-capp-dark text-2xl uppercase mb-2">No Profile Yet</h2>
        <p className="font-[Montserrat] text-sm text-capp-dark/55 mb-6 leading-relaxed">
          Create your counselor profile to start applying to camps.
        </p>
        <button
          onClick={() => navigate('/counselors/apply')}
          className="bg-capp-coral text-capp-dark font-[Montserrat] font-bold px-6 py-3.5 rounded-2xl active:scale-95 transition-transform"
        >
          Create Profile
        </button>
        <button onClick={() => navigate(-1)} className="mt-4 font-[Montserrat] text-sm text-capp-dark/40">← Go back</button>
      </div>
    )
  }
      ...
      <div className="bg-capp-dark px-4 pt-12 pb-8">
        ...
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-capp-coral flex items-center justify-center">
            <span className="font-[League_Spartan] font-bold text-capp-dark text-2xl">{initials}</span>
          </div>
          <div>
            <h1 className="font-[League_Spartan] font-bold text-white text-xl uppercase">{name}</h1>
            <p className="font-[Montserrat] text-xs text-white/55 mt-0.5">
              {profile.grade ? `${profile.grade} Grade` : ''}{profile.school ? ` · ${profile.school}` : ''}
            </p>
            {profile.city && (
              <p className="font-[Montserrat] text-xs text-white/40 mt-0.5">📍 {profile.city}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-5 flex flex-col gap-4">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-[League_Spartan] font-bold text-capp-dark text-base uppercase">My Applications</h2>
            <button
              onClick={() => navigate('/counselors/apply')}
              className="font-[Montserrat] text-xs font-bold text-capp-coral"
            >
              ＋ Apply Again
            </button>
          </div>
          ...
          {apps.length === 0 ? (
            <div className="bg-white rounded-2xl p-5 text-center shadow-sm">
              <p className="font-[Montserrat] text-sm text-capp-dark/50 mb-3">No applications yet.</p>
              <button
                onClick={() => navigate('/camps')}
                className="font-[Montserrat] text-sm font-semibold text-capp-coral"
              >
            ...
            <div className="flex flex-col gap-3">
              {apps.map(app => {
                const camp = app.campId ? camps.find(c => c.id === Number(app.campId)) : null
                  ...
                  <div key={app.id} className="bg-white rounded-2xl p-4 shadow-sm">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-[Montserrat] text-xs text-capp-dark/40 mt-0.5">Submitted {date}</p>
                      </div>
                    </div>
                    <div className="mt-3">
                      <span className={`inline-flex font-[Montserrat] text-xs font-semibold px-2.5 py-1 rounded-full border ${cfg.bg} ${cfg.text} ${cfg.border}`}>
                        {cfg.label}
                      </span>
                    </div>
                  </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-[League_Spartan] font-bold text-capp-dark text-base uppercase">Profile</h2>
            <button
              onClick={() => navigate('/counselors/apply')}
              className="font-[Montserrat] text-xs font-bold text-capp-dark/40"
            >
              Edit ✏️
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm divide-y divide-capp-dark/6">
            ...
            {profile.interests?.length > 0 && (
              <div className="px-4 py-3">
                <p className="font-[Montserrat] text-xs text-capp-dark/45 uppercase tracking-wider mb-2">Interests</p>
                <div className="flex flex-wrap gap-1.5">
                  {profile.interests.map(i => (
                    <span key={i} className="font-[Montserrat] text-xs font-semibold bg-capp-coral/15 text-capp-dark px-2.5 py-1 rounded-full">{i}</span>
                  ))}
                </div>
              </div>
            )}
        ...
        >
          ...
          <div className="flex-1 text-left">
            <p className="font-[Montserrat] text-sm font-bold text-white">Apply to a specific camp</p>
            <p className="font-[Montserrat] text-xs text-white/50 mt-0.5">Browse camps and apply directly</p>
          </div>
...
function Row({ label, value }) {
  ...
  return (
    <div className="px-4 py-3 flex items-start justify-between gap-4">
      <p className="font-[Montserrat] text-xs text-capp-dark/45 uppercase tracking-wider shrink-0 pt-0.5">{label}</p>
      <p className="font-[Montserrat] text-sm text-capp-dark text-right">{value}</p>
    </div>
  )
}
