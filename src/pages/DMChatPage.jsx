  ...
  if (!mom) {
    return (
      <div className="min-h-screen bg-capp-warm-bg flex items-center justify-center">
        <p className="font-garet text-capp-dark/50">Conversation not found</p>
      </div>
    )
  }

  // Find the shared camp (how they met)
  const sharedCamp = null
  ...
  return (
    <div className="flex flex-col h-screen bg-capp-warm-bg">
      ...
      <div className="flex items-center gap-3 px-4 pt-12 pb-3 border-b border-capp-dark/5 bg-white">
        ...
        <div
          className="w-11 h-11 rounded-full flex items-center justify-center text-lg font-bold text-white font-garet shrink-0 shadow-sm"
          style={{ backgroundColor: mom.avatarColor }}
        >
          {mom.name[0]}
        </div>
        <div className="flex-1 min-w-0">
          <h2 className="font-garet font-bold text-capp-dark text-base leading-tight uppercase">{mom.name}</h2>
          <p className="font-garet text-xs text-capp-dark/50">{mom.location} · {mom.kidName}'s mom</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-1" style={{ paddingBottom: '8rem' }}>
        ...
        {sharedCamp && (
          ...
          >
            ...
            <span className="font-garet text-xs font-medium text-capp-dark/60">
        ...
        {groups.map((item, i) => {
          ...
          return (
            <div key={`run-${i}`} className={`flex gap-2 mb-2 ${item.isMe ? 'justify-end' : 'justify-start'}`}>
              {!item.isMe && (
                <div
                  className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white font-garet shrink-0 mt-1"
                  style={{ backgroundColor: mom.avatarColor }}
                >
                  {mom.name[0]}
                </div>
              ...
              <div className={`flex flex-col gap-1 max-w-[78%] ${item.isMe ? 'items-end' : 'items-start'}`}>
                {item.messages.map((msg) => (
                  <div key={msg.id}>
                    ...
                    <p className={`font-garet text-[9px] text-capp-dark/30 mt-0.5 ${item.isMe ? 'text-right' : 'text-left ml-1'}`}>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center py-12 text-center">
            <div
              className="w-16 h-16 rounded-full flex items-center justify-center text-3xl font-bold text-white font-garet mb-3 shadow-md"
              style={{ backgroundColor: mom.avatarColor }}
            >
              {mom.name[0]}
            </div>
            <p className="font-garet font-bold text-capp-dark text-lg">{mom.name}</p>
            <p className="font-garet text-sm text-capp-dark/50 mt-1">{mom.location}</p>
            <p className="font-garet text-xs text-capp-dark/35 mt-3">
      ...
      {/* Input bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-capp-dark/8 px-3 pt-2 pb-2" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 72px)' }}>
        {/* Quick action chips */}
        <div className="flex gap-2 mb-2">
          <button
            onClick={() => sendMessage(convId, '🚗 I can offer a ride! Let me know if you need one.')}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 font-garet text-xs font-semibold text-emerald-700 active:scale-95 transition-transform"
          >
            🚗 Offer a Ride
          </button>
          <button
            onClick={() => sendMessage(convId, '🙏 Could we get a ride? Happy to return the favor!')}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-200 font-garet text-xs font-semibold text-blue-700 active:scale-95 transition-transform"
          >
            🙏 Request a Ride
          </button>
        </div>
