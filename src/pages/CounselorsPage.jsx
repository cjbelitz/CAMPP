  ...
  return (
    <div className="min-h-screen bg-capp-warm-bg pb-nav">
      ...
      <div className="relative bg-capp-dark px-5 pt-14 pb-12 overflow-hidden">
        ...
        <h1 className="font-[League_Spartan] font-bold text-white text-3xl leading-tight uppercase mb-2">
          Work at Camp This Summer
        </h1>
        <p className="font-[Montserrat] text-sm text-white/65 leading-relaxed mb-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            className="w-full py-3.5 rounded-2xl bg-white/10 text-white font-[Montserrat] font-semibold text-sm active:scale-95 transition-transform"
          >
      ...
      <div className="px-4 py-6 flex flex-col gap-6">
        ...
        {/* ── Benefits ── */}
        <section>
          <h2 className="font-[League_Spartan] font-bold text-capp-dark text-xl uppercase mb-4">Why counsel at camp?</h2>
          <div className="grid grid-cols-2 gap-3">
            {BENEFITS.map(b => (
              <div key={b.title} className="bg-white rounded-2xl p-4 shadow-sm">
                <span className="text-2xl block mb-2">{b.icon}</span>
                <p className="font-[Montserrat] text-sm font-semibold text-capp-dark leading-tight mb-1">{b.title}</p>
                <p className="font-[Montserrat] text-xs text-capp-dark/55 leading-relaxed">{b.body}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ── How it works ── */}
        <section>
          <h2 className="font-[League_Spartan] font-bold text-capp-dark text-xl uppercase mb-4">How it works</h2>
          <div className="flex flex-col gap-3">
            {HOW.map((h, i) => (
              <div key={h.step} className="bg-white rounded-2xl p-4 shadow-sm flex items-start gap-4">
                <span
                  className="font-[League_Spartan] font-bold text-sm w-9 h-9 rounded-xl bg-capp-coral/15 flex items-center justify-center shrink-0 text-capp-coral"
                >
                  {h.step}
                </span>
                <div>
                  <p className="font-[Montserrat] text-sm font-semibold text-capp-dark leading-tight mb-0.5">{h.title}</p>
                  <p className="font-[Montserrat] text-xs text-capp-dark/55 leading-relaxed">{h.body}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ── FAQ ── */}
        <section>
          <h2 className="font-[League_Spartan] font-bold text-capp-dark text-xl uppercase mb-4">Common questions</h2>
          <div className="bg-white rounded-2xl shadow-sm divide-y divide-capp-dark/6">
            {FAQ.map(f => (
              <div key={f.q} className="px-4 py-4">
                <p className="font-[Montserrat] text-sm font-semibold text-capp-dark mb-1">{f.q}</p>
                <p className="font-[Montserrat] text-xs text-capp-dark/60 leading-relaxed">{f.a}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="bg-capp-dark rounded-2xl p-5 text-center">
          <p className="font-[League_Spartan] font-bold text-white text-lg uppercase mb-2">Ready to make this summer count?</p>
          <p className="font-[Montserrat] text-xs text-white/55 mb-4">Applications take about 3 minutes on your phone.</p>
          <button
            onClick={() => navigate('/counselors/apply')}
            className="w-full py-4 rounded-2xl bg-capp-coral text-capp-dark font-[Montserrat] font-bold text-sm active:scale-95 transition-transform"
          >
