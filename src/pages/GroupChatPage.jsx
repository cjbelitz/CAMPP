...
function MomBubble({ color, initial, size = 8 }) {
  return (
    <div
      className={`w-${size} h-${size} rounded-full flex items-center justify-center text-xs font-bold text-white font-[DM_Sans] shrink-0`}
...
export default function GroupChatPage() {
  ...
  const displayAccent = camp?.accent ?? circleState.circleAccent ?? '#FFD166'
  ...
  if (!conv && !camp && !circleState.circleName) {
    return (
      <div className="min-h-screen bg-capp-warm-bg flex items-center justify-center">
        <p className="font-[DM_Sans] text-capp-dark/50">Circle not found</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-screen bg-capp-warm-bg">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-[Fraunces] font-bold text-capp-dark text-base leading-tight truncate">
            {displayName}
          </h2>
          <p className="font-[DM_Sans] text-xs text-capp-dark/50">
            group · {members.length + 1} members
          </p>
        </div>
        <button
          onClick={() => setShowMembers(true)}
          className="shrink-0 font-[DM_Sans] text-xs font-semibold text-capp-dark/50 bg-white/70 px-3 py-1.5 rounded-xl"
        >
          👥 {members.length + 1}
        </button>
      </div>
            ...
            return (
              <div key={`carpool-${i}`} className={`flex gap-2 mb-3 ${isMe ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`flex flex-col gap-1 max-w-[80%] ${isMe ? 'items-end' : 'items-start'}`}>
                  {!isMe && (
                    <span className="font-[DM_Sans] text-[10px] font-semibold text-capp-dark/50 ml-1">
                      {mom?.name}
                    </span>
                  )}
                  <CarpoolCard msg={item.msg} isMe={isMe} />
                  <p className={`font-[DM_Sans] text-[9px] text-capp-dark/30 mt-0.5 ${isMe ? 'text-right' : 'text-left ml-1'}`}>
      ...
      {showMembers && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div className="relative w-full bg-white rounded-t-3xl px-5 pt-5 pb-10">
            ...
            <h3 className="font-[Fraunces] font-bold text-capp-dark text-lg mb-4">
              {members.length + 1} members in this circle
            </h3>
            {/* Me */}
            <div className="flex items-center gap-3 py-2.5 border-b border-capp-dark/5">
              <div className="w-10 h-10 rounded-full bg-capp-coral flex items-center justify-center text-capp-dark text-sm font-bold font-[Fraunces]">
                Y
              </div>
              <div>
                <p className="font-[DM_Sans] text-sm font-semibold text-capp-dark">You</p>
                <p className="font-[DM_Sans] text-xs text-capp-dark/40">North County SD</p>
              </div>
            </div>
            {members.map((mom) => (
              <div key={mom.id} className="flex items-center gap-3 py-2.5 border-b border-capp-dark/5 last:border-0">
                ...
                <button
                  ...
                  className="font-[DM_Sans] text-xs font-semibold text-capp-coral bg-capp-coral/10 px-3 py-1.5 rounded-xl active:scale-95 transition-transform"
                >
                  DM
                </button>
              </div>
